...
class DropoutUncertaintyLSTMEncoder(nn.Module):
    ...
    def data_enc_for_model(self, data):
        """
        ...
        """
        cats = [
            data[0][i] for i in self.data_indices_enc[0]
        ]  # dims: list (n categorical values): Each with Tensor: batch_size x (window_size - suffix size)
        nums = [
