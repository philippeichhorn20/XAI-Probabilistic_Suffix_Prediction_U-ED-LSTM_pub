...
def _evaluate_single(batch_folder: str, filename: str, metrics: dict):
        ...
        for (case_name, prefix_len), (
            ...
            for metric_name, metric_obj in metrics.items():
                result = evaluate_metric(
                    metric_obj, prefix, suffix, mean_prediction, predicted_suffixes
                )
...
def evaluate_sequentially(batch_folder: str, metrics: dict[str, object]):

    pickle_files = sorted(f for f in os.listdir(batch_folder) if f.endswith(".pkl"))
    prefix_count, suffix_count = collections.defaultdict(
        lambda: 0
    ), collections.defaultdict(lambda: 0)
    results = {metric_name: {} for metric_name in metrics.keys()}
    for filename in pickle_files:
        new_results, (pc, sc) = _evaluate_single(batch_folder, filename, metrics)
...
def batch_evaluate(batch_folder: str, metrics: dict[str, object], num_workers: int = 4):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
        # Schedule the _evaluate_single calls concurrently
        futures = {
            executor.submit(_evaluate_single, batch_folder, filename, metrics): filename
            for filename in pickle_files
        }
