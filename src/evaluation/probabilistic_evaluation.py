...
class ProbabilisticEvaluation(Evaluation):
    ...
    def sample_cat_predictions_optim(
        ...
        for c in all_cat_attributes:
            if use_variance_cat:
                sampled_cats = torch.normal(
                    cat_means[c + "_mean"], torch.exp(cat_variances[c + "_var"])
                )
            else:
    ...
    def _evaluate_single(
        self, case_name, prefix_len, prefix, suffix, include_model_states
    ):
        ...
        predicted_suffixes = self.predict_probabilistic_suffix(
            prefix, prefix_len, include_model_states
        )
